...
function IDEWorkspace({ project, onNewProject }) {
  ...
  const loadFile = useCallback(async (path) => {
    ...
    try {
      const url = BACKEND_URL + "/api/project/" + project.project_id + "/file?path=" + encodeURIComponent(path);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.error) {
          ...
          const time = data.execution_time ? data.execution_time.toFixed(3) : "0";
          output = output + "\n✅ Execution successful (" + time + "s)";
  ...
  const canRunFile = selectedFile && (selectedFile.endsWith('.py') || selectedFile.endsWith('.js'));
        ...
        <div className="flex items-center gap-2">
          ...
          {selectedFile && (
            <Button variant="ghost" size="sm" onClick={analyzeCurrentFile} disabled={isAnalyzing} className="gap-2">
              <Bug className="w-4 h-4" /> Find Bugs
            </Button>
          )}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="flex-1">
            {isLoadingFile ? (
              ...
              <React.Suspense fallback={<div className="h-full flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin" /></div>}>
                <MonacoEditor
                  height="100%"
